...
function createScript(){
    ...
    newScriptTag.setAttribute("id", "jsonp");
    
    replaceScript(newScriptTag)  
}

function replaceScript(newScriptTag){
    let oldScriptElement= document.getElementById("jsonp");
    let body= document.getElementsByTagName("body")[0];
    if(oldScriptElement== null){
        body.appendChild(newScriptTag);
    }else{
        body.replaceChild(newScriptTag, oldScriptElement)
    }
}
...
function addDataToLineChart(sale){ 
    if (data.labels.length=== 10 && data.datasets[0].data.length===10){
        data.datasets[0].data.shift();
        data.labels.shift();
    } 
    data.datasets[0].data.push(sale.sales);
    data.labels.push(sale.name);
}
...
function updateLatestSales(sale){
    updateLatestSaleUno(sale);
    updateLatestSaleDue(sale);
};

function updateLatestSaleUno(sale){
...
function updateLatestSaleDue(sale){
